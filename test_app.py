...
# app.py checks that the advisor package on the server is new enough for it.
# That check is only useful if the symbols it names are real: a typo there would
# stop a perfectly good deployment with a message about a function that never
# existed.
import importlib as _importlib

_check_src = _re.search(r"_REQUIRED_SYMBOLS = \[(.*?)\n\]",
                        pathlib.Path("app.py").read_text(), _re.S).group(1)
_checked = _re.findall(r'\("(advisor/[a-z_]+\.py)",\s*\w+,\s*\[(.*?)\]\)',
                       _check_src, _re.S)
check("the deployment check names files that exist",
      all(pathlib.Path(f).is_file() for f, _ in _checked),
      f"{len(_checked)} modules checked")

_bad = []
for _file, _names in _checked:
    _mod = _importlib.import_module(
        "advisor." + pathlib.Path(_file).stem)
    for _n in _re.findall(r'"([^"]+)"', _names):
        if not hasattr(_mod, _n):
            _bad.append(f"{_file}:{_n}")
check("every symbol the deployment check requires actually exists",
      not _bad, ", ".join(_bad) or
      f"{sum(len(_re.findall(chr(34) + '([^' + chr(34) + ']+)' + chr(34), n)) for _, n in _checked)} symbols")

# The check has to cover the modules app.py newly depends on, or a partial
# upload still dies with a redacted AttributeError somewhere further in.
_checked_files = {f for f, _ in _checked}
check("the modules app.py gained new dependencies on are all covered",
      {"advisor/data.py", "advisor/charts.py", "advisor/metrics.py",
       "advisor/report.py", "advisor/th.py"} <= _checked_files,
      ", ".join(sorted(_checked_files)))
check("a missing report.py is survivable at import time",
      "except ImportError:" in pathlib.Path("app.py").read_text().split(
          "from advisor import report")[1][:200])

# Everything the deployed app needs, so a manifest can be checked against it.
_MANIFEST = ["app.py", "requirements.txt", "advisor", "assets/fonts",
             "assets/fonts/Sarabun-Regular.ttf", "assets/fonts/Sarabun-Bold.ttf",
             "assets/fonts/OFL.txt", "data/Fund Return.xlsx"]
_absent = [f for f in _MANIFEST if not pathlib.Path(f).exists()]
check("every file the deployment manifest lists is present",
      not _absent, ", ".join(_absent) or f"{len(_MANIFEST)} entries")
check("requirements names the two deploy-only dependencies",
      all(pkg in pathlib.Path("requirements.txt").read_text()
          for pkg in ("reportlab", "kaleido")))
