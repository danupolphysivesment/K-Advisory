...
# Performance
# --------------------------------------------------------------------------- #
# The growth curve answers "what would this book have done with my money in
# it?", so it is based on an investment amount rather than a unit NAV — ten
# million baht, the order of magnitude these clients actually invest, which
# makes every point on the curve a number they can read as money.
GROWTH_BASE = 10_000_000.0


def baht_label(amount: float) -> str:
    """``10 ล้านบาท`` / ``500,000 บาท`` — an amount as an RM would say it."""
    if amount >= 1_000_000 and amount % 1_000_000 == 0:
        return f"{amount / 1_000_000:,.0f} ล้านบาท"
    if amount >= 1_000_000:
        return f"{amount / 1_000_000:,.1f} ล้านบาท"
    return f"{amount:,.0f} บาท"


def growth_chart(series_map: Mapping[str, pd.Series], height: int = 340,
                 base: float = GROWTH_BASE, log: bool = False) -> go.Figure:
    if not series_map:
        return _empty()
    fig = go.Figure()
    for i, (label, series) in enumerate(series_map.items()):
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate="<b>%{fullData.name}</b><br>%{x|%d %b %Y}<br>"
                          "%{y:,.0f} บาท<extra></extra>",
        ))
    fig.update_layout(height=height,
                      yaxis=dict(title=f"มูลค่าเงินลงทุน เริ่มต้นที่ "
                                       f"{baht_label(base)}",
...
def drawdown_chart(series_map: Mapping[str, pd.Series], height: int = 250) -> go.Figure:
    ...
    fig.update_layout(height=height, yaxis=dict(tickformat=".0%", title="Drawdown"),
...
def risk_vs_weight(frame: pd.DataFrame, labels: Optional[Mapping[str, str]] = None,
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=ordered["weight"], y=names, orientation="h", name="Capital Weight",
        marker=dict(color=T.TEAL), width=0.38, offset=-0.40,
        hovertemplate="<b>%{y}</b><br>Weight %{x:.1%}<extra></extra>",
    ))
    fig.add_trace(go.Bar(
        x=ordered["risk_share"], y=names, orientation="h", name="Risk Contribution",
...
def correlation_heatmap(corr: pd.DataFrame, labels: Optional[Mapping[str, str]] = None,
    ...
    fig = go.Figure(go.Heatmap(
        ...
        colorscale=T.CORRELATION_SCALE, zmid=0, zmin=-1, zmax=1,
        hovertemplate="%{y} กับ %{x}<br>ρ = %{z:.2f}<extra></extra>",
        colorbar=dict(outlinewidth=0, tickfont=dict(color=T.MUTED, size=10),
                      thickness=10, len=0.8),
    ))

    # Plotly 5.9 takes only a scalar textfont colour for a heatmap, and one
    # colour cannot work across a diverging scale — light text vanishes on the
    # bright end, dark text on the dark end. Annotations are per-cell, so each
    # number is drawn in whichever of the two the fill under it can carry.
    for row, y_name in enumerate(names):
        for col, x_name in enumerate(names):
            value = float(corr.values[row, col])
            fig.add_annotation(
                x=x_name, y=y_name, text=f"{value:.2f}".rstrip("0").rstrip(".")
                if abs(value) < 1 else f"{value:.0f}",
                showarrow=False,
                font=dict(family=T.SANS, size=10,
                          color=T.on_scale(T.CORRELATION_SCALE,
                                           (value + 1.0) / 2.0)))
...
def fan_chart(percentiles: pd.DataFrame, period_label: str,
    ...
    for lo, hi, alpha in bands:
        ...
        fig.add_trace(go.Scatter(
            ...
            name=f"Percentile {lo[1:]}–{hi[1:]}", showlegend=True,
        ))
    fig.add_trace(go.Scatter(
        x=x, y=scaled["p50"], mode="lines", name="Median",
...
def frontier_chart(frontier: pd.DataFrame,
    ...
    for label, vol, ret, colour, symbol in points:
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f"<b>{label}</b><br>Volatility %{{x:.2%}}<br>"
                          f"Return %{{y:.2%}}<extra></extra>",
        ))
    fig.update_layout(height=height, showlegend=False,
                      xaxis=dict(title="Expected Volatility", tickformat=".0%"),
...
def core_satellite_bar(split: Mapping[str, float], height: int = 120) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=[core], y=[""], orientation="h", name="Core",
                         marker=dict(color=T.TEAL),
                         text=[f"Core {core:.0%}"], textposition="inside",
                         textfont=dict(family=T.SANS, size=12, color=T.TEXT),
                         hovertemplate="Core %{x:.1%}<extra></extra>"))
    fig.add_trace(go.Bar(x=[sat], y=[""], orientation="h", name="Satellite",
                         marker=dict(color=T.SAND),
                         text=[f"Satellite {sat:.0%}"], textposition="inside",
                         textfont=dict(family=T.SANS, size=12, color=T.BG),
                         hovertemplate="Satellite %{x:.1%}<extra></extra>"))
