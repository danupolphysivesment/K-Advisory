...
def monthly_return_table(returns: pd.Series) -> pd.DataFrame:
    ...
    m = (1.0 + returns.dropna()).resample("M").prod() - 1.0
...
def summary_frame(series_map: Dict[str, pd.Series], rf: float = DEFAULT_RF) -> pd.DataFrame:
    ...
    for label, series in series_map.items():
        s = compute(series, rf)
        rows[label] = {
            "ผลตอบแทนต่อปี (CAGR)": s.cagr,
            "ความผันผวน": s.volatility,
            "Sharpe": s.sharpe,
            "Sortino": s.sortino,
            "ขาดทุนสูงสุด (Max drawdown)": s.max_drawdown,
            "Calmar": s.calmar,
            "VaR 95% (1 วัน)": s.var_95,
            "CVaR 95% (1 วัน)": s.cvar_95,
            "ความเบ้ (Skew)": s.skew,
            "ความโด่งส่วนเกิน (Kurtosis)": s.kurtosis,
            "สัดส่วนวันที่เป็นบวก": s.hit_rate,
        }
