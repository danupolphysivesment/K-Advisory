...
GBM = "Geometric Brownian Motion (การแจกแจงปกติ)"
STUDENT_T = "Student-t (หางหนา)"
BOOTSTRAP = "สุ่มจากข้อมูลจริง (Bootstrap)"
BLOCK = "สุ่มเป็นช่วง (Block bootstrap)"
REGIME = "สลับสถานะตลาด 2 สถานะ (Regime-switching)"

METHODS: List[str] = [GBM, STUDENT_T, BOOTSTRAP, BLOCK, REGIME]
...
METHOD_NOTES: Dict[str, str] = {
    ...
    BLOCK: "สุ่มช่วงเวลาที่ต่อเนื่องกัน ทำให้การกระจุกตัวของความผันผวนและแนวโน้ม "
           ...
           "เมื่อต้องการตอบคำถามเรื่องการขาดทุนสูงสุด",
...
def simulate(
    ...
    if mu_override:
        ...
        mu_source = "สมมติฐานตลาดทุนแบบมองไปข้างหน้า"
...
def compare_methods(
    ...
    **kwargs,
    ...
    for m in methods:
        ...
        rows[m] = {
            "มูลค่าปลายทาง (กลาง)": s["median"],
            "เปอร์เซ็นไทล์ที่ 5": s["p5"],
            "เปอร์เซ็นไทล์ที่ 95": s["p95"],
            "ผลตอบแทนต่อปี (กลาง)": s["median_cagr"],
            "โอกาสขาดทุน": s["prob_loss"],
            "ขาดทุนสูงสุดโดยทั่วไป": s["median_max_dd"],
            "ขาดทุนสูงสุด 1 ใน 100": s["worst_max_dd"],
        }
