...
<style>
  ...
  /* ---------- tabs ----------
     Top-level navigation between three views of one client, so it is styled as
     a set of section headings rather than a control: the label carries the
     weight and a mint rule underlines the one you are reading. The indicator is
     drawn as a ::after bar instead of border-bottom so it can sit *on* the
     hairline, keep rounded ends, and animate its width — a border cannot do
     any of the three. Streamlit puts the label in its own <p>, which carries
     the body text colour, so colour and weight are set on that element; setting
     them on the tab alone leaves every tab looking selected. */
  .stTabs [data-baseweb="tab-list"] {{
    gap: 0.15rem; background: transparent; padding: 0;
    border-bottom: 1px solid {LINE}; margin-bottom: 0.5rem;
    /* Streamlit sets overflow-x: scroll on this row. Three Thai labels are wide
       enough to overflow a narrow window, and the resulting scrollbar cuts
       across the hairline. Keep it scrollable, hide the furniture. */
  ...
  .stTabs [data-baseweb="tab"] {{
    ...
    border-radius: 3px 3px 0 0; padding: 0.62rem 1.25rem 0.66rem;
    color: {MUTED}; transition: color 140ms ease, background 140ms ease;
  }}
  .stTabs [data-baseweb="tab"] p {{
    color: {MUTED}; font-family: {DISPLAY}; font-size: 0.92rem;
    font-weight: 500; letter-spacing: 0.005em; margin: 0;
    transition: color 140ms ease;
  }}
  /* The indicator: a hairline-wide bar that grows out from the centre. */
  .stTabs [data-baseweb="tab"]::after {{
    content: ""; position: absolute; left: 50%; bottom: -1px;
    width: 0; height: 2px; border-radius: 2px; background: {MINT};
    transform: translateX(-50%);
    transition: width 180ms cubic-bezier(0.4, 0, 0.2, 1);
  }}
  .stTabs [data-baseweb="tab"]:hover {{ background: {CARD}66; }}
  .stTabs [data-baseweb="tab"]:hover p {{ color: {TEXT}; }}
  .stTabs [data-baseweb="tab"]:hover::after {{ width: 28%; background: {LINE}; }}
  ...
  .stTabs [aria-selected="true"]:hover::after {{
    width: calc(100% - 2.5rem); background: {MINT};
  }}
  .stTabs [data-baseweb="tab"]:focus-visible {{
    outline: none; box-shadow: 0 0 0 2px {MINT}44; border-radius: 3px;
  }}
  .stTabs [data-baseweb="tab-highlight"] {{ display: none; }}
  .stTabs [data-baseweb="tab-border"] {{ display: none; }}

  /* On a laptop with the sidebar open the content column is narrow enough that
     the three labels no longer fit. Tighten the tabs rather than let them
     scroll — a tab an RM cannot see is a tab they will not open. */
  @media (max-width: 1200px) {{
    .stTabs [data-baseweb="tab"] {{ padding: 0.6rem 0.7rem 0.64rem; }}
