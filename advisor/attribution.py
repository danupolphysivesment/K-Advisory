...
def contribution_over_time(
    ...
    """Cumulative contribution by group through time, for a stacked area chart."""
    codes = [c for c in weights if c in returns.columns and weights[c] > 0]
    if not codes:
        return pd.DataFrame()
    w0 = np.array([weights[c] for c in codes], dtype=float)
    ...
    r = panel.fillna(0.0).to_numpy()
    idx = panel.index
    period = pd.Series(idx, index=idx).dt.to_period(
        period_alias(rebalance if rebalance not in ("D", "none") else "Q"))
    marks = writable(period.ne(period.shift()).to_numpy())
    marks[0] = False

    value = 1.0
    w = w0.copy()
    rows = np.zeros((len(idx), len(codes)))
    for t in range(len(idx)):
        if marks[t] and rebalance not in ("D", "none"):
            w = w0.copy()
        if rebalance == "D":
            w = w0.copy()
        rows[t] = value * w * r[t]
        grown = w * (1.0 + r[t])
        value = value * grown.sum()
        w = grown / grown.sum() if grown.sum() > 0 else w0.copy()

    frame = pd.DataFrame(rows, index=idx, columns=codes)
    labels = pd.Index([group_of.get(c, "Other") for c in codes], name="group")
    frame = frame.T.groupby(labels).sum().T
    return frame.resample(resample_alias(freq)).sum().cumsum()
...
def policy_benchmark(profile_bands: Mapping[str, tuple],
                     bucket_proxies: Mapping[str, str]) -> Dict[str, float]:
    """A neutral policy portfolio sitting at the mid-point of each SEC band.

    Used as the benchmark for Brinson attribution: it is the allocation a
    compliant, opinion-free portfolio for this risk profile would hold.
    """
    mids = {}
    for bucket, (lo, hi) in profile_bands.items():
        mids[bucket] = (lo + hi) / 2.0
    total = sum(mids.values())
    if total <= 0:
        return {}
    out: Dict[str, float] = {}
    for bucket, share in mids.items():
        proxy = bucket_proxies.get(bucket)
        if proxy:
            out[proxy] = out.get(proxy, 0.0) + share / total
    return out
