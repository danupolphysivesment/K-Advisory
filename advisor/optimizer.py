...
# Not an objective — the label a hand-built portfolio carries so that every
# chart, table and export downstream has something to print where the objective
# would go.
CUSTOM = "Custom (กำหนดเอง)"

# Every objective the solver implements. Kept whole so the code stays covered
# by the test suite and so re-offering one is a single-line change.
...
# What the app actually offers an RM. The naive-weighting methods (inverse
# volatility, risk parity, equal weight) and the two tail/downside variants
# were dropped from the menu: they are useful as academic benchmarks but they
# are not portfolios anyone here intends to recommend, and a menu of eleven
# invites the RM to shop for the flattering number.
OBJECTIVES: List[str] = [
    MAX_SHARPE, MIN_VOL, TARGET_RETURN, TARGET_VOL, MIN_DRAWDOWN,
    MAX_DIVERSIFICATION,
]
...
def evaluate(
    ...
    """Score a portfolio the RM built by hand, using the solver's own maths.

    The optimiser and a hand-built book must be measured the same way or the
    comparison between them is meaningless, so this shares every formula with
    :func:`optimise` and differs only in where the weights came from.

    ``cons`` is read as a *checklist*, not a constraint set: a hand-built
    portfolio is allowed to break the mandate, and the RM is told which rules it
    breaks rather than having the position quietly moved. Suitability bands are
    checked separately in ``clients.check_suitability`` — this covers the house
    rules (per-fund cap, satellite cap, holding count).
    """
    held = {c: float(w) for c, w in weights.items()
            if c in returns.columns and float(w) > 1e-9}
    if not held:
        return Solution({}, label, 0.0, 0.0, 0.0, "error",
                        "ยังไม่ได้เลือกกองทุน กรุณาเลือกอย่างน้อย 1 กองทุน")

    total = sum(held.values())
    if total <= 0:
        return Solution({}, label, 0.0, 0.0, 0.0, "error",
                        "น้ำหนักรวมต้องมากกว่า 0%")
    # Normalise rather than refuse: the UI shows the raw total and offers to fix
    # it, and every statistic below is only defined on a fully invested book.
    held = {c: w / total for c, w in held.items()}
    codes = sorted(held)
    ...
    if len(panel) < 30:
        return Solution({}, label, 0.0, 0.0, 0.0, "error",
                        "ข้อมูลย้อนหลังที่ทับซ้อนกันของกองทุนกลุ่มนี้ไม่เพียงพอ "
                        "ลองเอากองทุนที่เพิ่งจัดตั้งออก")
    ...
    w = np.array([held[c] for c in codes], dtype=float)
    mu_vec = mu.reindex(codes).fillna(0.0).to_numpy()
    R = panel.to_numpy(dtype=float)

    exp_ret = float(w @ mu_vec)
    v = portfolio_volatility(w, cov.to_numpy())
    sharpe = (exp_ret - rf) / v if v > 0 else 0.0

    breaches: List[str] = []
    if cons is not None:
        heaviest = max(held.items(), key=lambda kv: kv[1])
        if heaviest[1] > cons.max_weight + 1e-6:
            breaches.append(
                f"{universe[heaviest[0]].name} ถือ {heaviest[1]:.0%} "
                f"เกินเพดานต่อกองทุนที่ตั้งไว้ {cons.max_weight:.0%}")
        sat = float(core_satellite_split(held, universe)["Satellite"])
        if sat > cons.max_satellite + 1e-6:
            breaches.append(
                f"สัดส่วน Satellite รวม {sat:.0%} "
                f"เกินเพดานที่ตั้งไว้ {cons.max_satellite:.0%}")
        if cons.max_funds and len(held) > cons.max_funds:
            breaches.append(
                f"ถือ {len(held)} กองทุน มากกว่าจำนวนเป้าหมาย {cons.max_funds} กอง")
        above = sum(w for c, w in held.items() if c in cons.above_level_codes)
        if above > cons.above_level_budget + 1e-6:
            breaches.append(
                f"ผลิตภัณฑ์ที่เสี่ยงสูงกว่าระดับของลูกค้ารวม {above:.0%} "
                f"เกินวงเงินที่ลูกค้ารับทราบ {cons.above_level_budget:.0%}")
    ...
    diagnostics = {
        ...
        "history_end": panel.index[-1],
        "history_periods": len(panel),
        "eligible_funds": len(codes),
        "feasible": not breaches,
        "max_drawdown_sample": _max_drawdown_of(w, R),
        "raw_total": total,
        "mandate_breaches": breaches,
    }
    if breaches:
        message = ("พอร์ตนี้ไม่ตรงกับ Mandate ที่ตั้งไว้ — "
                   + " · ".join(breaches)
                   + " ระบบไม่ได้แก้ให้ เพราะเป็นพอร์ตที่ผู้แนะนำกำหนดเอง")
    else:
        message = "พอร์ตที่กำหนดเองนี้อยู่ในกรอบ Mandate ทุกข้อ"
    return Solution(held, label, exp_ret, v, sharpe,
                    "warning" if breaches else "ok", message, diagnostics)
