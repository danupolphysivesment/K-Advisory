...
# Objectives
# --------------------------------------------------------------------------- #
MAX_SHARPE = "Sharpe ratio สูงสุด"
MIN_VOL = "ความผันผวนต่ำสุด"
TARGET_RETURN = "ผลตอบแทนเป้าหมาย"
TARGET_VOL = "ความผันผวนเป้าหมาย"
MIN_DRAWDOWN = "ขาดทุนสูงสุดต่ำที่สุด"
MIN_CVAR = "CVaR (95%) ต่ำสุด"
INVERSE_VOL = "ถ่วงน้ำหนักผกผันความผันผวน"
RISK_PARITY = "Risk parity (กระจายความเสี่ยงเท่ากัน)"
MAX_DIVERSIFICATION = "กระจายความเสี่ยงสูงสุด"
MAX_SORTINO = "Sortino ratio สูงสุด"
EQUAL_WEIGHT = "ถ่วงน้ำหนักเท่ากัน"
...
OBJECTIVE_NOTES: Dict[str, str] = {
    ...
    MIN_DRAWDOWN: "ลดการขาดทุนจากจุดสูงสุดถึงจุดต่ำสุดที่เกิดขึ้นจริงในข้อมูล "
    ...
    MIN_CVAR: "ลดค่าเฉลี่ยการขาดทุนในช่วงที่แย่ที่สุด 5% แก้เป็นโปรแกรมเชิงเส้น "
              ...
              "แบบสมมาตร",
    INVERSE_VOL: "น้ำหนักแปรผันกับ 1/σ ไม่ต้องใช้ตัวหาค่าเหมาะสม และไม่มี"
...
def resolve_bands(
    ...
    if not res.success:
        return BandResolution(dict(bands), False, [],
                              ...
                              "หรือขยายจักรวาลกองทุนที่ลงทุนได้")
...
def optimise(
    ...
    if objective == EQUAL_WEIGHT:
        ...
        status = "ok" if ok else "warning"
        if w is not None:
            message = f"ขาดทุนสูงสุดที่เกิดขึ้นจริงในข้อมูล: {_max_drawdown_of(w, R):.1%}"
    ...
    if w is None or not np.all(np.isfinite(w)):
        return Solution(
            ...
            "กรุณาเพิ่มเพดานน้ำหนักต่อกองทุน ขยายจักรวาลกองทุนที่ลงทุนได้ "
...
def trade_list(current: Mapping[str, float], proposed: Mapping[str, float],
    ...
    for c in codes:
        ...
        rows.append({
            ...
            "กลุ่มสินทรัพย์": fund.asset_class if fund else "",
            "ปัจจุบัน": now,
            "เสนอใหม่": new,
            "เปลี่ยนแปลง": delta,
