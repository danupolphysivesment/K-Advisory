...
# The caution cards read display fields that were added to Theme and Signal
# after the first release. Reading them through helpers means a half-deployed
# checkout degrades to a plainer card instead of failing the whole tab.
REQUIRED_SCHEMA = 2


def _clip(text: str, limit: int = 110) -> str:
    """Cut at the last space before ``limit`` — never mid-word.

    Thai has almost no inter-word spaces, so a word-count truncation returns
    the whole paragraph. Character-based cutting at a space is the only thing
    that behaves for both scripts.
    """
...
def theme_caution(theme) -> str:
    """The card sentence, falling back to a clipped explainer if absent."""
    written = getattr(theme, "caution", "")
    return written or _clip(getattr(theme, "why", "") or theme.title)


def signal_reading(signal) -> str:
    """The bare number behind a signal, or "" when the field is unavailable."""
    if signal is None:
        return ""
    return getattr(signal, "reading", "") or ""
...
class Caution:
    ...
    headline: str              # one plain sentence: what this does to the money
    reading: str               # the live number behind it, or "" when there is none
...
def build_cautions(
    ...
    for tv in view.themes:
        ...
        # The card copy is written for the card. Earlier versions truncated the
        # long ``why`` explainer, which does not work in Thai — the language has
        # almost no spaces, so a word-count truncation returns the whole
        # paragraph and the card becomes unreadable.
        reading = signal_reading(tv.signal)
        caution = theme_caution(theme)
        message = f"{caution} (กระทบ {exposure:.0%} ของพอร์ต: {names})"
...
def headline_risk_summary(cautions: Sequence[Caution]) -> str:
    ...
    top = cautions[0]
    if len(cautions) == 1:
        return (f"เรื่องที่ควรคุยกับลูกค้ารายนี้คือ {top.short} "
                f"ซึ่งกระทบ {top.exposure:.0%} ของพอร์ต")
    second = cautions[1]
    return (f"เรื่องที่ควรคุยกับลูกค้ารายนี้มี 2 เรื่อง คือ {top.short} "
            f"(กระทบ {top.exposure:.0%} ของพอร์ต) และ {second.short} "
            f"(กระทบ {second.exposure:.0%})")
...
def structural_observations(
    ...
    if len(live) == 1:
        ...
        out.append(Observation(
            ...
            f"Effective Positions อยู่ที่ {effective_bets:.1f} "
            f"ยังมีช่องให้กระจายความเสี่ยงเพิ่มได้โดยไม่เปลี่ยนลักษณะของพอร์ต"))

    if diversification_ratio < 1.15:
        out.append(Observation(
            "diversification", "high",
            f"Diversification Ratio อยู่ที่ {diversification_ratio:.2f} "
