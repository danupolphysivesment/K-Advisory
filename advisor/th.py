...
# Suitability findings carry their own two-level severity, lower case and
# separate from the market-theme scale above. On screen it only picks a colour;
# in an exported document it has to be a word the client can read.
...
# Portfolio roles
# --------------------------------------------------------------------------- #
# Shown where a field genuinely has no value rather than a value of zero —
# a blank cell reads as a rendering fault, and a zero reads as a measurement.
...
def finding_severity(value: str) -> str:
    return FINDING_SEVERITY.get(str(value).lower(), value)
