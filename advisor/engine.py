...
def comparison_table(
    ...
    rows = [
        ("ผลตอบแทนต่อปี", sa.cagr, sb.cagr, "pct", "higher"),
        ("ความผันผวน", sa.volatility, sb.volatility, "pct", "lower"),
        ("Sharpe ratio", sa.sharpe, sb.sharpe, "num", "higher"),
        ("Sortino ratio", sa.sortino, sb.sortino, "num", "higher"),
        ("ขาดทุนสูงสุด", sa.max_drawdown, sb.max_drawdown, "pct", "higher"),
        ("Calmar ratio", sa.calmar, sb.calmar, "num", "higher"),
        ("วันที่แย่ที่สุด", sa.worst_period, sb.worst_period, "pct", "higher"),
        ("VaR 95% ราย 1 วัน", sa.var_95, sb.var_95, "pct", "higher"),
        ("CVaR 95% ราย 1 วัน", sa.cvar_95, sb.cvar_95, "pct", "higher"),
        ("สัดส่วนวันที่เป็นบวก", sa.hit_rate, sb.hit_rate, "pct", "higher"),
        ("จำนวนสถานะที่มีผลจริง", current.effective_bets, proposed.effective_bets,
         "num", "higher"),
        ("อัตราส่วนการกระจายความเสี่ยง", current.diversification_ratio,
