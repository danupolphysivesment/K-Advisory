...
def to_monthly(returns: pd.DataFrame | pd.Series) -> pd.DataFrame | pd.Series:
    ...
    return (1.0 + returns).resample(resample_alias("M")).prod() - 1.0
...
def portfolio_returns(
    ...
    if rebalance == "none":
        ...
        # writable(): on pandas with Copy-on-Write, .to_numpy() can return a
        # read-only view of the Series buffer, and assigning into it raises.
...
def first_day(returns: pd.DataFrame, code: str) -> Optional[pd.Timestamp]:
    """The first date ``code`` has a return for, or ``None`` if it has none."""
    if code not in returns:
        return None
    series = returns[code].dropna()
    return pd.Timestamp(series.index[0]) if not series.empty else None


def common_floor(returns: pd.DataFrame,
                 codes: Iterable[str]) -> Optional[pd.Timestamp]:
    """The first day every one of ``codes`` has data.

    A portfolio is only as long as its newest holding: a book containing a fund
    launched in 2023 has no 2016, whatever the workbook holds for its other
    funds. Windows are floored here so a header cannot state a period the
    numbers underneath it do not cover.
    """
    days = [d for d in (first_day(returns, c) for c in codes) if d is not None]
